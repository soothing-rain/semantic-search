...
class MilvusHelper:
    ...
    def __init__(self):
        try:
            ...
            LOGGER.debug(
                f"Successfully connect to Milvus with IP:{MILVUS_HOST,} and "
                f"PORT:{MILVUS_PORT}")
        ...
        try:
            if not self.has_collection(collection_name):
                field1 = FieldSchema(name="id", dtype=DataType.INT64,
                                     descrition="int64", is_primary=True,
                                     auto_id=True)
    ...
    def insert(self, collection_name, data):
        ...
        try:
            ...
            LOGGER.debug(
                f"Insert vectors to Milvus in collection: {collection_name} with {len(data[0])} rows")
            return ids
        except Exception as e:
